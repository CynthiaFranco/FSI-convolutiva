...
with tf.Session() as sess:
    ...
    while True:
        ...
        if (abs(error - errorPrev) / errorPrev) < 0.0005 or epoch > 500:
            break
        epoch = epoch + 1

    aciertos = 0.
    testResult = sess.run(example_batch_test_predicted)
    print ("****** TEST *****")
    for etiqueta, y in zip(label_batch_test.eval(), testResult):
        if (np.argmax(etiqueta) == np.argmax(y)):
            aciertos = aciertos + 1
    tasaAcierto = aciertos / len(label_batch_test.eval()) * 100
    print ("La tasa de acierto obtenida es: ", tasaAcierto, "%")

    plt.plot(errors)
    plt.title("Evolucion del error de validacion")
    plt.xlabel("Epocas")
    plt.ylabel("Error")
    plt.savefig("graficaTest.png")
    plt.show()
